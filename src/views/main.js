...
/*window.localStorage.removeItem('isLoggedIn');
   ...
   if (isIPad) {
      ...
      var textAddToHomescreen = new Surface({
         content: 'Add this app to your homescreen.',
         size: [230, null],
         properties: {
            zIndex: '100',
            font: '15px Helvetica',
      ...
      var textAddToHomescreenModifier = new StateModifier({
         align: [0.5, 0.5],
         origin: [0.5, 0.5],
         opacity: 0
      });

      mainContext.add(textAddToHomescreenModifier).add(textAddToHomescreen);

      if (true) {
         var appView = new AppView();
         mainContext.add(appView);
      } else {
         textAddToHomescreenModifier.setOpacity(1);
      }
